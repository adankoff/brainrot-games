/**
 * FLAPPY DOGE -- UI Components
 * ScoreDisplay (on-canvas HUD).
 */
...
export class ScoreDisplay {
   ...
   * @param {string} [scoreColor] - Override score color per theme
   * @param {string} [flashColor] - Override flash color per theme
   */
  draw(ctx, canvasWidth, scoreColor, flashColor) {
    const fontSize = Math.round(56 * this.popScale);

    let color = scoreColor || COLOR_SCORE;
    if (this.score === 69 && this.flashTimer > 0) {
      color = COLOR_SCORE_69;
    } else if (this.flashTimer > 0) {
      color = flashColor || COLOR_SCORE_FLASH;
    }
