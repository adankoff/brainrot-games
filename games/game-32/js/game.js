...
export class SpawnScheduler {
  ...
  selectCharacterType(effectiveWave) {
    ...
    weights.brrbrr = getPenaltyWeight(effectiveWave);

    // Zero out characters that haven't appeared yet (redundant but safe)
    if (effectiveWave < CHARACTER_TYPES.lirili.appearsFromWave) {
      weights.lirili = 0;
    }
    ...
    if (total === 0) return CHARACTER_TYPES.tralalero; // fallback
    ...
    // Fallback
    return CHARACTER_TYPES.tralalero;
  }
