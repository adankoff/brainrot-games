...
export function createSliceParticles(x, y, themeId) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (themeId === 'classic-meme') {
      // "LOL" text particles burst out
      const texts = ['LOL', 'XD', 'LMAO', 'KEK', 'ROFL'];
      opts.text = texts[Math.floor(Math.random() * texts.length)];
      opts.color = '#333333';
    } else if (themeId === 'anime-slash') {
      // Speed line burst particles
      opts.color = '#ffffff';
      opts.size = randomBetween(3, 7);
    } else if (themeId === 'cat-slice') {
      // Feather/yarn colored particles
      const colors = ['#ff69b4', '#ff99cc', '#ffb6c1', '#dda0dd', '#f0e68c'];
