/**
 * ANIME CLICKER -- Main Entry Point
 * Creates GameShell, wires callbacks, manages game state.
 * Idle clicker with 3 swappable anime theme variants.
 * Cloned from game-03 (Sigma Grindset Simulator).
 */
...
// Milestone tracking (Dragon Ball "IT'S OVER 9000!" etc.)
...
const shell = new GameShell({
  ...
  maxDisplayWidth: 480,
  theme: 'anime-clicker',
  subtitle: 'power up. evolve. ascend.',
  accentColor: '#ffd700',
  shareUrl: 'https://brainrotgames.com/games/game-18/',
});
...
shell.onStart = () => {
  ...
  saveTimer = 0;
  elapsedTime = 0;
  lastMilestoneIndex = calcMilestoneIndex();
};
...
shell.onRender = (ctx) => {
  // Background
  drawBackground(ctx, currentTheme.colors);

  // Currency display
  const production = getTotalProduction(currentTheme.upgrades, ownedCounts, prestigeTokens);
  drawCurrencyDisplay(ctx, currentTheme, currency, production, prestigeTokens);

  // Tap target (pass totalEarned for Dragon Ball aura tiers)
  drawTapTarget(ctx, currentTheme, bounceScale, elapsedTime, totalEarned);

  // Milestone flash (Dragon Ball "IT'S OVER 9000!")
  drawMilestoneFlash(ctx, currentTheme.colors);
...
shell.onGameOver = () => {
  ...
  return {
    ...
    message: 'You ascended with ' + tokensGained + ' ' + currentTheme.prestigeName + '!',
...
// We use onTap solely for audio init on first interaction.
input.onTap(() => {
  if (!audioInitialized) {
    try {
      initAudio();
...
function doTap(pos) {
  ...
  // Track taps for JoJo pose cycling
  registerTap();
...
// ---- Milestone System (Dragon Ball) ----

/**
 * Calculate the current milestone index based on totalEarned.
 *
 * @returns {number} Index into milestones array, or -1 if none reached
 */
function calcMilestoneIndex() {
  if (!currentTheme.milestones) return -1;
  const milestones = currentTheme.milestones;
  let idx = -1;
  for (let i = 0; i < milestones.length; i++) {
    if (totalEarned >= milestones[i]) idx = i;
  }
  return idx;
}

/**
 * Check if a new milestone has been reached and trigger the flash.
 */
function checkMilestones() {
  if (!currentTheme.milestones) return;
  const idx = calcMilestoneIndex();
  if (idx > lastMilestoneIndex) {
    lastMilestoneIndex = idx;
    const val = currentTheme.milestones[idx];
    triggerMilestoneFlash("IT'S OVER " + formatNumber(val) + '!');
  }
}
...
function switchTheme(themeId) {
  ...
  // Reset milestone tracking for new theme
  lastMilestoneIndex = -1;
