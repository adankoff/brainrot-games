...
export function renderFrame(ctx, board, theme, anim) {
  ...
  // Italian theme: subtle flag stripes at top and bottom
  if (theme.id === 'italian') {
    ...
    ctx.fillStyle = '#009246';
    ctx.fillRect(0, H - stripeH, W / 3, stripeH);
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(W / 3, H - stripeH, W / 3, stripeH);
    ctx.fillStyle = '#ce2b37';
    ctx.fillRect(W * 2 / 3, H - stripeH, W / 3, stripeH);
    ctx.globalAlpha = 1;
  }

  // Among Us theme: star dots in background
  if (theme.id === 'amongus') {
    ctx.fillStyle = '#ffffff';
    for (let i = 0; i < 30; i++) {
