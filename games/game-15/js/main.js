/**
 * NYAN CAT RUN -- Main Entry Point
 * Creates GameShell, wires callbacks, manages game state.
 * Side-scrolling endless runner with 3 swappable cat meme themes.
 * Cloned from game-04 (Ohio Survival Run).
 */
...
// Theme-specific gravity values
const THEME_GRAVITY = {
  nyancat: 0.4,   // Lower gravity -- Nyan Cat flies through space
  maxwell: 0.6,   // Normal gravity
  keyboard: 0.6,  // Normal gravity
};
...
function loadTheme() {
  ...
  player.setGravity(THEME_GRAVITY[id] || 0.6);
...
const shell = new GameShell({
  ...
  maxDisplayWidth: 480,
  theme: 'cat-run',
  subtitle: 'nyan nyan nyan nyan nyan.',
  accentColor: '#ff69b4',
  shareUrl: 'https://brainrotgames.com/games/game-15/',
});
...
shell.onStart = () => {
  ...
  // Apply theme gravity to new player
  const themeId = getSelectedThemeId();
  player.setGravity(THEME_GRAVITY[themeId] || 0.6);
...
function drawHUD(ctx) {
  ...
  ctx.fillStyle = currentTheme ? currentTheme.accentColor : '#ff69b4';
