 ...
 * Full 360-degree roll on jump for the "YOU DIED" knight dodge-roll feel.
 *
...
export function playerJump(player) {
  ...
  player.targetRotation += Math.PI * 2; // Full 360 roll per jump
}
...
export function updatePlayer(player, obstacles, dt) {
  ...
  // Ground collision
  if (!overGap && player.y + PLAYER_SIZE >= GROUND_Y) {
    player.y = GROUND_Y - PLAYER_SIZE;
    player.vy = 0;
    player.grounded = true;
  } else if (overGap && player.y + PLAYER_SIZE >= GROUND_Y) {
    // Falling into a gap
    player.grounded = false;
  }
  ...
  if (player.grounded) {
    // Snap to nearest full rotation when grounded
